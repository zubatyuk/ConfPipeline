...
def TurboDesc(mol, script_gas, script_cosmo=None, n_jobs=1):
    ...
    results = Parallel(n_jobs=n_jobs)(delayed(run_calc)(
        *[
            (write_xyz, {'mol': mol, 'conf_id': conf_id, 'fil': 'tm.xyz'}),
            (os.system,  [script_gas + ' tm.xyz']),
            (parse_cosmo, {}),
            (os.system if script_cosmo else noop, [script_cosmo + ' tm.xyz']),
            (get_tm_energy if script_cosmo else noop, {}),
            (parse_cosmo, {}),
        ]
    ) for conf_id in conf_ids)
    mol.p_gas = [dict(Vol=x[2][0], **politzer(x[2][2], x[2][3])) for x in results]
    if script_cosmo:
        mol.energies_cosmo = [x[4] for x in results]
        mol.p_cosmo = [dict(Vol=x[5][0], **politzer(x[5][2], x[5][3])) for x in results]
    return mol
...
def parse_cosmo():
    ...
    with open('energy') as fil:
        for line in fil:
            if not vol and line.startswith('  volume='):
            ...
            elif not N and line.startwith('  nps='):
                ...
                for i in range(N):
                    ...
                    area[i] = ll[6]
                    potential[i] = ll[8]
    return vol, coord, area, potential
